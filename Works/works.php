<?php
/*
Plugin Name: WE Boracay Works
Description: Declares a plugin that will create a custom post type displaying weboracay works.
Version: 1.0
...
add_action( 'init', 'works' );
	...
	add_action( 'init', 'works_taxonomies', 1 );
    
	function works_taxonomies() {
		$labels = array(
...
function columns_content_works($column_name, $post_ID) {
    ...
    } else if ( $column_name == 'event-date' ) {
...
add_action('manage_works_posts_custom_column', 'columns_content_works', 10, 2); 
 
 function works_post_columns( $talents_columns ) {
    $columns = array (
        'cb' => '<input type="checkbox" />',
        'event_image' => __( 'Featured Image' ),
        'title' => __( 'Event Name' ),
        'location' => __( 'Location' ),
        'taxonomy-works_category' => __( 'Categories' ),
        'event-date' => __( 'Event Date' ),
        'date' => __( 'Date' )
    );
    return $columns;
}
add_filter( 'manage_works_posts_columns', 'works_post_columns' );
?>